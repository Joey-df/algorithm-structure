...
public class Code07_MaxSumInTree_3 {
    ...
    public static Info process(TreeNode x) {
        ...
        if (leftInfo != null && rightInfo != null) {
            p6 = x.val + leftInfo.fromHeadMax + rightInfo.fromHeadMax;
        }

        int allMax = Math.max(Math.max(p1, Math.max(p2, Math.max(p3, p4))), Math.max(p5, p6));
